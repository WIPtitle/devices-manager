...
class SensorsListenerImpl(SensorsListener):
    def __init__(self, alarm_manager: AlarmManager, sensor_repository: SensorRepository,
        ...
        for sensor in self.sensor_repository.find_all():
            try:
                ...
                initial_state = self.gpio_monitor_client.get_pin_state(sensor.gpio_pin_number)
                self.sensor_states[sensor.gpio_pin_number] = initial_state
                # Publish initial state
                event_manager.publish_sensor_event_sync(sensor.gpio_pin_number, initial_state)
            except Exception as e:
                print(f"Failed to initialize sensor on pin {sensor.gpio_pin_number}: {e}")
    ...
    def add_sensor(self, sensor: Sensor):
        ...
        try:
            ...
            # Publish initial state for new sensor
            event_manager.publish_sensor_event_sync(sensor.gpio_pin_number, state)
        except Exception as e:
    ...
    def update_sensor(self, sensor: Sensor):
        ...
        try:
            state = self.gpio_monitor_client.get_pin_state(sensor.gpio_pin_number)
            old_state = self.sensor_states.get(sensor.gpio_pin_number)
            self.sensor_states[sensor.gpio_pin_number] = state
    ...
    def get_status_by_sensor(self, sensor: Sensor) -> int:
        ...
        try:
            state = self.gpio_monitor_client.get_pin_state(sensor.gpio_pin_number)
            self.sensor_states[sensor.gpio_pin_number] = state
            print(f"State for pin {sensor.gpio_pin_number} was not cached, fetched: {state}")
            # Publish the newly fetched state
            event_manager.publish_sensor_event_sync(sensor.gpio_pin_number, state)
            return state
        except Exception as e:
            print(f"Failed to get status for sensor on pin {sensor.gpio_pin_number}: {e}")
    ...
    def _register_sensor_callback(self, sensor: Sensor):
        ...
        def on_state_change(pin: int, state: int):
            old_state = self.sensor_states.get(pin)
            # Update local state
            self.sensor_states[pin] = state

            # Publish event if state actually changed
            if old_state != state:
                print(f"Sensor on pin {pin} state changed from {old_state} to {state}")
                event_manager.publish_sensor_event_sync(pin, state)
            ...
            if state == 1:  # HIGH state
                try:
                    ...
                    if sensor_db.listening:
                        print(f"Sensor on pin {pin} triggered (state=HIGH)")
                        self.alarm_manager.on_sensor_triggered(pin)
                except Exception as e:
                    print(f"Error handling state change for pin {pin}: {e}")
