...
def read_current_status(gpio_pin_number: int, value_when_closed: GpioValue) -> ReedStatus:
    pull = GPIO.PUD_DOWN if value_when_closed == GpioValue.HIGH else GPIO.PUD_UP
    GPIO.setup(gpio_pin_number, GPIO.IN, pull_up_down=pull)
    current_value = GPIO.input(gpio_pin_number)

    if (value_when_closed == GpioValue.HIGH and current_value == GPIO.HIGH) or \
            (value_when_closed == GpioValue.LOW and current_value == GPIO.LOW):
...
class ReedsListenerImpl(ReedsListener):
    def __init__(self, alarm_manager: AlarmManager, reed_repository: ReedRepository):
        ...
        self.reed_infos: Dict[int, Tuple[GpioValue, ReedStatus]] = {}
        self.running = True
    ...
    def add_reed(self, reed: Reed):
        if self.reed_infos.get(reed.gpio_pin_number) is None:
            value_when_closed = reed.default_value_when_closed
            self.reed_infos[reed.gpio_pin_number] = (
                value_when_closed,
                read_current_status(reed.gpio_pin_number, value_when_closed)
            )
        else:
    ...
    def update_reed(self, reed: Reed):
        if self.reed_infos.get(reed.gpio_pin_number) is None:
            ...
            self.reed_infos[reed.gpio_pin_number] = (
                reed.default_value_when_closed,
                read_current_status(reed.gpio_pin_number, reed.default_value_when_closed)
            )
    ...
    def get_status_by_reed(self, reed: Reed) -> ReedStatus:
        if self.reed_infos.get(reed.gpio_pin_number) is None:
            ...
            return self.reed_infos.get(reed.gpio_pin_number)[1]


    def monitor_pins(self):
        while self.running:
            for pin in self.reed_infos.keys():
                current_status = read_current_status(pin, self.reed_infos.get(pin)[0])
                if current_status != self.reed_infos.get(pin)[1]:
                    self.reed_infos[pin] = (
