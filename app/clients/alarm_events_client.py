...
class AlarmEventsClient:
    ...
    def notify_alarm_waiting(self, started: bool, duration: int = None):
        ...
        response = httpx.post(self._audio_url("/internal/alarm/waiting"), json=body, timeout=TIMEOUT)
        response.raise_for_status()

    def notify_alarm_stopped(self):
        """Notify audio service that alarm has stopped."""
        response = httpx.post(self._audio_url("/internal/alarm/stopped"), timeout=TIMEOUT)
        response.raise_for_status()
    ...
    def _notify_audio_sensor_alarm(self, sensor_name: str, duration: int = None):
        ...
        if duration is not None:
            body["duration"] = duration
        response = httpx.post(self._audio_url("/internal/alarm/sensor-alarm"), json=body, timeout=TIMEOUT)
        response.raise_for_status()

    def _notify_notifications_sensor_alarm(self, sensor_name: str):
        """Notify notifications service about sensor alarm."""
        response = httpx.post(self._notif_url("/internal/alarm/sensor-alarm"), json={"sensor_name": sensor_name}, timeout=TIMEOUT)
        response.raise_for_status()

    def notify_motion_warning_audio(self, camera_name: str):
        """Notify audio service about motion warning (immediate sound)."""
        try:
            httpx.post(self._audio_url("/internal/alarm/motion-warning"), json={"camera_name": camera_name}, timeout=TIMEOUT)
    ...
    def send_motion_notification(self, camera_name: str, snapshot_jpeg: bytes | None = None):
        ...
        try:
            ...
            httpx.post(self._notif_url("/internal/alarm/motion-warning"), data={"camera_name": camera_name}, files=files, timeout=TIMEOUT)
        except Exception as e:
